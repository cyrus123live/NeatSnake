...
def getSnakeInfo(snake):
    ...
    x = int(snake.segments[len(snake.segments) - 1][0])
    y = int(snake.segments[len(snake.segments) - 1][1])
    ...
    if snake.direction == "right":
        ...
        for i in range(WIDTH - x):
            if dist_straight_tail != -1:
                break
            for j in range(snake.size - 1):
                seg = snake.segments[len(snake.segments) - 2 - j]
                if seg[0] == x + i and seg[1] == y and dist_straight_tail == -1:
                    dist_straight_tail = i - SNAKESIZE
                    break
        dist_right_tail = -1
        for i in range(HEIGHT - y):
            if dist_right_tail != -1:
                break
            for j in range(snake.size - 1):
                seg = snake.segments[len(snake.segments) - 2 - j]
                if seg[0] == x and seg[1] == y + i and dist_right_tail == -1:
                    dist_right_tail = i - SNAKESIZE
        dist_left_tail = -1
        for i in range(y):
            if dist_left_tail != -1:
                break
            for j in range(snake.size - 1):
                seg = snake.segments[len(snake.segments) - 2 - j]
                if seg[0] == x and seg[1] == y - i and dist_left_tail == -1:
                    dist_left_tail = i - SNAKESIZE
        ...
        dist_right_fruit = snake.fruit[0] - x - SNAKESIZE
        ...
        for i in range(y):
            ...
            for j in range(snake.size - 1):
                seg = snake.segments[len(snake.segments) - 2 - j]
                if seg[0] == x and seg[1] == y - i and dist_straight_tail == -1:
                    dist_straight_tail = i - SNAKESIZE
        dist_right_tail = -1
        for i in range(WIDTH - x):
            if dist_right_tail != -1:
            ...
            for j in range(snake.size - 1):
                seg = snake.segments[len(snake.segments) - 2 - j]
                if seg[0] == x + i and seg[1] == y and dist_right_tail == -1:
                    dist_right_tail = i - SNAKESIZE
        dist_left_tail = -1
        for i in range(x):
            if dist_left_tail != -1:
            ...
            for j in range(snake.size - 1):
                seg = snake.segments[len(snake.segments) - 2 - j]
                if seg[0] == x - i and seg[1] == y and dist_left_tail == -1:
                    dist_left_tail = i - SNAKESIZE
    ...
    info = [dist_straight_wall, dist_straight_fruit, dist_straight_tail, dist_right_wall, dist_right_fruit, dist_right_tail, dist_left_wall, dist_left_fruit, dist_left_tail]
...
def eval_genomes(genomes, config):
    ...
    while True:
        ...
        for i, snake in enumerate(snakes):
            ...
            # #trim snake's segments
            # if len(snake.segments) > 700:
            #     for i in range(snake.segments):
            #         if i < 350:
            #             del(snake.segments[i])
            ...
            if killed or ge[i].fitness >= config.fitness_threshold:
                del(snakes[i])
                del(ge[i])
                del(nets[i])
                #print(f"snake {i} has died")
        ...
        if len(snakes) == 1 and not(simulated) and generation % VISIBLEGENERATION == 0 and bool(draw):
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(local_dir, 'configCollisions')
